...
const Project = ({ title, description, tags, imageUrl }: ProjectClass) => {
  ...
  return (
    ...
    >
      <section
        className="bg-gray-100 max-w-[42rem] borderBlack/5 overflow-hidden sm:pr-8 relative sm:h-[20rem] 
     sm:group-even:pl-8 hover:bg-gray-200 transition hover:drop-shadow-xl rounded-lg dark:bg-white/10 dark:hover:bg-white/20
     dark:text-white"
      >
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {tags.map((tag, index) => (
              <li
                className="bg-black/[.7] px-3 py-1 text-[.7rem] uppercase tracking-wider text-white rounded-full dark:text-white/70"
                key={index}
              >
                {tag}
              </li>
        ...
        group-hover:-translate-x-3 group-hover:translate-y-3 transition group-hover:-rotate-2 group-hover:scale-[1.04]
